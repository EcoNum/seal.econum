...
# Amélioration de fonctions

## idées 1 changer calb pour n'avoir que la visualisation 

```{r}
SciViews::R
library(seal.econum)
# import raw data and the supplement file  (xlsx format) 
aa3_txt <- "../inst/extra_data/181018E.TXT"
aa3_xlsx <- "../inst/extra_data/181018E.xlsx"
# use the function to import and convert data
aa3_combine <- convert_aa3(file_aa3_txt = aa3_txt,
                           file_aa3_xlsx = aa3_xlsx)
# others informations
class(aa3_combine)
attributes(aa3_combine)
```
...
En ajoutant l'ensemble des éléments dans une liste, on obtient la fonction plot_aa3()

```{r}
plot_aa3(aa3_combine)
```

Après avoir visualiser les deux graphiques, ainsi que le tableau de régression, la phase de validation débute.

On décide sur base de la gamme de concentrations de nos échantillons de recalculer les droites de calibration. 

```{r}
# vali <- validate_data_aa3(aa3_combine, filter_list = list(Ptot = c(25, 50), Ntot = c(0.1, 0.5, 1, 2), NO2 = 10))
```

```{r}
# vali
# attributes(vali)
# #vali$regression$NO2$param
```
